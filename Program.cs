    ...
    Application // this is where the comparison application itself starts, the top code will get moved to other files
{
        ...
        for (int r = 0; r < a.GetLength(0); r++) {
            for (int c = 0; c < a.GetLength(1); c++) {
                if (a[r, c] - b[r, c] > maxOk || b[r, c] - a[r, c] > maxOk) {
                    throw new ArgumentException("a[" + r + ", " + c + "] is not equal to " +
                                                "b[" + r + ", " + c + "]. " +
    ...
    private Timers.Stopwatch _stopwatch = new Timers.Stopwatch(); // store the timers here, they will reset whenever .Start() is executed
    private Timers.CPU _cpu = new Timers.CPU();
    private int algorithmsMaxIncrements = 4; // the amount of times we want to increment by nodesIncrementAmount, to test performance with smaller & larger graphs
    ...
    public TestResults CreateTestResults(string algorithmName, double stopwatchElapsedMilliseconds, double cpuElapsedMilliseconds, int nodesAmount)
    {
    ...
    public void Execute()
    {
        ...
        for (int i = 1; i < algorithmsMaxIncrements + 1; i++) // + 1, we don't want to start from 0 since then we'll have 0 nodes
        {
            Graph graph = new Graph(nodesIncrementAmount * i, 50); // here, we create the graph that we're going to be using for the algorithms with x nodes and y percent (which determines the amount of possible edges)
            
            StartTimers();
            double[,] floydDistances = FloydWarshallShortestPath.Execute(graph); // run floyd warshall's algorithm and store it in a variable 
            StopTimers();
            ...
            StartTimers();
            double[,] dijkstraDistances = DijkstraShortestPath.Execute(graph); // same as in the above one, we store the variable for interpretation
            StopTimers();
            
            double dijkstraStopwatchTime = _stopwatch.GetElapsedMilliseconds();
            double dijkstraCpuTime = _cpu.GetElapsedMilliseconds(); // we cache the variables here so we can add them to TestResults later, without the need of creating 2x timer objects
            Console.WriteLine("Dijkstra algorithm has finished running");
            
            try
            {
                VerifyDistances(floydDistances, dijkstraDistances); // verify to make sure the algorithms are functioning as expected, we do this in try & catch statements since we return an exception if it fails
        ...
        if (floydAlgorithmResults.Count > 0 && dijkstraAlgorithmResults.Count > 0) // only do this if we havent ran into any issues above, indicated by the fact we have more than 0 count
        {
            WriteResultsToCsv(fileName, floydAlgorithmResults);
            WriteResultsToCsv(fileName, dijkstraAlgorithmResults); // write results, it's appended so its fine to call it twice
        }
    }
}